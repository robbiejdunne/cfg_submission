...
def calc_player_velocities(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12, radius=1):
    ...
    for player in player_ids: # cycle through players individually
        ...
        within_r = 0
        px = team[player+"_x"].values[0]
        py = team[player+"_y"].values[0]
        d_px_bx = np.square(px - bx)
        d_py_by = np.square(py - by)
        try:
            if np.sqrt(d_px_bx + d_py_by) <= np.square(rad):
                within_r = True
            else:
                within_r = False
        except:
