...
@author: Edd Webster (@EddWebster) based on the original code by Laurie Shaw (@EightyFivePoint)

"""
import numpy as np
import scipy.signal as signal

def calc_player_velocities(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12, rad=2):
    ...
    bx = team['ball_x'].values[0]
    by = team['ball_y'].values[0]
    ...
    # Determine the velocities of the players and the distances they are from the ball
    for player in player_ids: # cycle through players individually
        
        # Determine whether players are within a defined radius of the ball
        within_r = False
        try:
            if np.sqrt((np.square((team[player+"_x"].values[0]) - bx)) + (np.square((team[player+"_y"].values[0]) - by))) <= rad:
                within_r = True
            else:
                within_r = False
        except:
        ...
        team[player + "_within_r"] = within_r
...
def remove_player_velocities(team):
    ...
    columns = [c for c in team.columns if c.split('_')[-1] in ['vx','vy','ax','ay','speed','acceleration','r']] # Get the player ids
